...
;; requires: (posn-x posn2)cannot equal 0 when (posn-y posn2)=0
;;           and vice versa
...
(check-expect (posn-div (make-posn 1 2) (make-posn 1 0)) (make-posn 1 2))
...
;;(posn-reciprocal position) determines the reciprocal of a posn
;; posn-reciprocal: Posn -> Posn
;; requires: (posn-x position) cannot equal 0 when (posn-y position)=0
;;           and vice versa
;;           position must only have Num values
